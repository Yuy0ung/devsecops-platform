...
func Start() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := redisdb.Client.RPush(ctx, "dast:queue:jobs", bytes).Err(); err != nil {
			// 此时 MySQL 已经是 running 了，如果推送失败，应该回滚状态？
			// 简单起见，这里记录日志或报错。实际生产应有更健壮的补偿。
...
func Stop() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 5) 尝试取消 scanner（发布消息到 Redis control 频道）
		// scanner.Cancel(taskId)
		controlMsg := map[string]string{
			"action": "cancel",
			"taskId": taskId,
		}
		cBytes, _ := json.Marshal(controlMsg)
		redisdb.Client.Publish(redisdb.Ctx, "dast:control", cBytes)
