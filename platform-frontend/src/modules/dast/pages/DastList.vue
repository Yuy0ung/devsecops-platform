...
const handleCreateTask = async () => {
  ...
  try {
    const res = await request.post("/api/dast/task/create", {
...
const handleStartTask = async (task: TaskItem) => {
  ...
  try {
    const res = await request.get("/api/dast/task/start", {
...
const handleStopTask = async (task: TaskItem) => {
  if (task.status !== "running") return;
  try {
    const res = await request.get("/api/dast/task/stop", {
      params: { taskId: task.taskId },
    });
...
const deleteTasks = async (taskIds: string[]) => {
  try {
    await Promise.all(
      taskIds.map((id) =>
        request.get("/api/dast/task/delete", {
          params: { taskId: id },
        })
      )
    );
