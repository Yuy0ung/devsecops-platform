...
const fetchTaskStatus = async () => {
  const res = await request.get("/api/dast/task/list");
...
const fetchTargets = async () => {
  const res = await request.get("/api/dast/target/list", {
...
const fetchResults = async () => {
  const res = await request.get("/api/dast/target/result", {
    params: {
      taskId: taskId.value,
...
const handleAddTargets = async () => {
  ...
  try {
    await request.post("/api/dast/target/add", {
      taskId: taskId.value,
...
const handleDeleteTargets = () => {
  ...
  Modal.confirm({
    ...
    async onOk() {
      try {
        await request.post("/api/dast/target/delete", {
          taskId: taskId.value,
          targets: deleteList,
        });
...
const handleSaveEditTargets = async () => {
  ...
  try {
    ...
    if (addTargets.length) {
      reqs.push(
        request.post("/api/dast/target/add", {
          taskId: taskId.value,
          targets: addTargets,
        })
      );
    ...
    if (deleteTargets.length) {
      reqs.push(
        request.post("/api/dast/target/delete", {
          taskId: taskId.value,
